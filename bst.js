...
class BinarySearchTree {
  ...
  remove(data) {
    ...
    var removeNode = (node, data) => {
        ...
        if(data === node.data) {
          ...
          // 2 children = find min of right branch and recursively call removeNode
          var temp = that.getMin(node.right);
          node.data = temp; // node.data = min of right tree
  ...
  _isBalanced(node) {
    ...
    var heighthLeft = this._getHeight(node.left);
    var heighthRight = this._getHeight(node.right);
    var diff = Math.abs(heighthLeft - heighthRight);
    if(diff > 1) {
      return false;
    } else {
